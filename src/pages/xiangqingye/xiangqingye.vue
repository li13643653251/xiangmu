<template>
	<view class="content" v-if="show">
		<view class="contenttop">
			...
			<view class="money ">
				<p class="paddingtext">￥{{money}}</p>
				<view class="yanseguige size14">
					<view class="leftimg">
					...
					<scroll-view style=" height: 150rpx;" scroll-x="true">
						<view style="width: 150%; display: flex;">
							<view class="leftimg" v-for="(item,index) in skuInfoEntities" :key="index" @tap="preview(index)">
								<img :src="item.skuDefaultImg" alt="">
								<p style="text-align: center;  word-wrap: break-word;
    word-break: normal;  width: 100%; height: 20px; font-size: 10px;" >{{item.skuAttrName}}</p>
							</view>
						</view>
					</scroll-view>
				</view>
			</view>
			<view class="xiangqingyevip paddingtext">
			...
			<view class="blacktext paddingtext">
				<view class="mix-icon icon-shoujihaoma "></view>
				<view style="color: #FFFFFF;" class="size12">下单后三个工作日内发货</view>
			</view>
			<view class="shoptitle jianju">
				<p>[双十一 狂欢季]Laneige 兰芝果冻夜间修护 睡眠唇膜韩国正品送迪迈诗隔离霜</p>
			</view>
		</view>

		<!-- 配送 -->
		<view class="peisong paddingtext">

			<p class="size14"> 配送 :至</p>
			<!-- <p> -->
		...
		<!-- 保障 -->
		<view class="baozhang">
			<view class="size14 paddingtext" @click="open('bottom')">保障: <span style="margin-left:5%;">正品保障 ○全场包邮 ○买贵就赔 ○7天无理由 </span>
				<span class="mix-icon icon-you" style="float: right;"></span></view>
		</view>
		<!-- msak -->
		...
		<!-- 起售说明 -->
		<view class="qishoushuoming paddingtext">
		...
		<!-- 商品评价 -->
		<view class="pingjia">
			<view class="pingjiatitle paddingtext">
		...
		<!-- 为你推荐 -->
		<view class="tuijian">
			<view class="tuijiantitle">○ 商品详情 ○</view>
	...
	export default {
		...
		methods: {
			...
			preview(index) {
				let _this = this;
				let imgsArray = [];
				for (let i = 0; i < this.skuInfoEntities.length; i++) {
					imgsArray.push(this.skuInfoEntities[i].skuDefaultImg);
				}
				...
				// console.log(item.skuDefaultImg)
			},
			// 加入购物车
			lijigoumai() {
				...
				// 	url:"http://www.panchaotian.com:1035/addTocart",
				// 	data:{
				// 		"weixinToken":q,
				// 		"memberId":uni.getStorageSync('token').id,
				// 		"skuId":this.xuanzhongshop[0].skuId,
				// 		"num":this.shopnum
			...
			addgwc(a, b) {
				console.log(this.xuanzhongshop)
			...
			shopdata() {
				this.$request("product/api/sku_info", {
					...
					if (res.skuAttrResps[0] == undefined) {
						console.log(res.skuAttrResps)
						this.show = false
						uni.showToast({
							title: '数据异常！',
							icon: "loading",
